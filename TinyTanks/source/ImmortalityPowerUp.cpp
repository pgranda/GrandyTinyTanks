...
*/
void ImmortalityPowerUp::ExecutePowerUpEvent(Tank* a_pTank)
{
	a_pTank->SetIsImmortal(true);
}
...
*/
void ImmortalityPowerUp::DisplayPowerUpInformation(float a_fGameplayScreenWidth, int a_iScreenHeight, float a_multiplicator)
{
	UG::SetFont("./fonts/invaders.fnt");
	UG::DrawString("     is Immortal!", (int)(a_fGameplayScreenWidth * a_multiplicator), a_iScreenHeight - 2, 0.7f, UG::SColour(255, 0, 0, 255));
}

ImmortalityPowerUp::ImmortalityPowerUp(const char* a_fileName, int a_width, int a_height, Vector2 a_position, float a_angle)

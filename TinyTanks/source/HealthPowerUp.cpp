...
*/
void HealthPowerUp::ExecutePowerUpEvent(Tank* a_pTank)
{
	a_pTank->SetHp(100);
}
...
*/
void HealthPowerUp::DisplayPowerUpInformation(float a_fGameplayScreenWidth, int a_iScreenHeight, float a_multiplicator)
{
	UG::SetFont("./fonts/invaders.fnt");
	UG::DrawString("     regained his Health!", (int)(a_fGameplayScreenWidth * a_multiplicator), a_iScreenHeight - 2, 0.7f, UG::SColour(255, 0, 0, 255));
}

HealthPowerUp::HealthPowerUp(const char* a_fileName, int a_width, int a_height, Vector2 a_position, float a_angle)
